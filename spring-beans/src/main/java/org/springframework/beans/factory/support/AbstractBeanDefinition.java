 ...
 * 具体的、成熟的{@link BeanDefinition}类的基类，分解出{@link GenericBeanDefinition}、{@link RootBeanDefinition}
...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	 ...
	 * 默认范围名的常量:{@code ""}，等同于单例状态，除非从父bean定义中覆盖(如果适用)
	 */
	...
	// autowire-candidate 属性设置为 false，这样容器在查找自动装配对象时，将不考虑该 bean，即
	// 它不会被考虑作为其他 bean 自动装配的候选者，但是该 bean 本身还是可以使用自动装配来注入其他的 bean
	...
	// 是否以一种宽松的模式解析构造函数，默认为 true，如果为 false，则在如下情况
	/**
	 * interface ITest{}
	 ...
	 *     Main(ITestImpl i){}
	 * }
	 * 会抛出异常，因为Spring无法准确定位哪个构造函数，程序设置
	 */
	...
	// 构造函数注入属性，对应 bean 属性 constructor-arg
	@Nullable
	...
	/**
	 * Validate and prepare the method overrides defined for this bean.(验证和准备为此bean定义的方法覆盖。)
	 * Checks for existence of a method with the specified name.(检查具有指定名称的方法的存在。)
	 * @throws BeanDefinitionValidationException in case of validation failure
	 */
