...
public class DefaultBeanDefinitionDocumentReader implements BeanDefinitionDocumentReader {
	 ...
	 * 根据 "spring-beans" XSD 或 DTD 实现解析 bean 定义
	 *
	...
	public void registerBeanDefinitions(Document doc, XmlReaderContext readerContext) {
		...
		// 提取 root，以便与再次将 root 作为参数继续 BeanDefinition 的注册
	...
	protected void doRegisterBeanDefinitions(Element root) {
		...
		// 任何嵌套的<beans>元素都会在此方法中导致递归。为了正确地传播和保存<beans> default-*属性，请跟踪当前(父)委托，该委托可能为空。
		// 创建新的(子)委托，并使用对父委托的引用来进行回退，然后最终将this.delegate重置为其原始的(父)引用。这种行为会模拟一个委托堆栈，
		// 而实际上并不需要一个。
	...
	protected void importBeanDefinitionResource(Element ele) {
		...
		if (absoluteLocation) {
			...
			try {
				...
				// 而每个 resource 的 createRelative 的实现方式都不一样，所以这里先使用子类的方法尝试解析
	...
	protected void processBeanDefinition(Element ele, BeanDefinitionParserDelegate delegate) {
		// 元素解析，经过此方法后，bdHolder 实例已经包含我们配置文件中 配置的各种属性了，如 class、name、id、alias之类的属性
		BeanDefinitionHolder bdHolder = delegate.parseBeanDefinitionElement(ele);
		if (bdHolder != null) {
			// bdHolder 不为空时，如果默认标签下有自定义属性，再次对自定义标签进行解析
